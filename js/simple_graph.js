...
var drawGraph = function() {
		...
		circles.forEach(t => {
            $(SVG('circle')).attr('cx', t[0]).attr('cy', t[1]).attr('r', 3).attr('fill', t[3]).attr('stroke', 'black').attr('stroke-width', '1').click(function() {
                if (lastAt != t[0]) {	
                    lastAt = t[0];
                    let e = $(this).position();
                    $('#hoverer').css({
					...
					$('#hoverer span:first-child').text(circle_ys[t[0]][0] + '%')
					$('#hoverer span:last-child').text(circle_ys[t[0]][1] + '°')
